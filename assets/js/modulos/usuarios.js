...
function setModulosChecked(modulos) {
  ['inspecciones','personal','reportes','matriz','amonestaciones','geocercas','evaluaciones','capacitaciones','checklist','kpi_analytics','epp','vehiculos','empresas','documentos'].forEach(m=>{const el=document.getElementById('mod_'+m);if(el)el.checked=modulos.includes(m);});
}
function getModulosSeleccionados() {
  const checked=['dashboard'];
  ['inspecciones','personal','reportes','matriz','amonestaciones','geocercas','evaluaciones','capacitaciones','checklist','kpi_analytics','epp','vehiculos','empresas','documentos'].forEach(m=>{const el=document.getElementById('mod_'+m);if(el&&el.checked)checked.push(m);});
  return checked;
}

  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:8}}>
        ...
        <button onClick={()=>onSave(form)} style={{padding:"8px 20px",borderRadius:4,border:"none",background:"#1ABB9C",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"'Barlow',sans-serif"}}>
  ...
  return (
    <div>
      <div style={{marginBottom:14,padding:"12px 16px",background:"#F5F7FA",borderRadius:4,border:"1px solid #E6E9ED"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          {[["replace","Reemplazar todo"],["append","Agregar al final"]].map(([v,l])=>(
            <button key={v} onClick={()=>setMode(v)} style={{padding:"6px 14px",borderRadius:4,fontSize:12,fontWeight:600,cursor:"pointer",border:`1px solid ${mode===v?"#1ABB9C":"#CDD3D8"}`,background:mode===v?"rgba(26,187,156,.08)":"none",color:mode===v?"#1ABB9C":"#73879C",fontFamily:"'Barlow',sans-serif"}}>{l}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:14}}>
        <button onClick={onCancel} style={{padding:"8px 18px",borderRadius:4,border:"1px solid #CDD3D8",background:"none",color:"#73879C",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"'Barlow',sans-serif"}}>Cancelar</button>
        <button onClick={()=>onConfirm(rows,mode)} style={{padding:"8px 20px",borderRadius:4,border:"none",background:"#26B99A",color:"#fff",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"'Barlow',sans-serif"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,marginBottom:14}}>
        <div>
          <h2 style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:22,fontWeight:800,color:"#2A3F54",textTransform:"uppercase",letterSpacing:"0.5px",margin:0,lineHeight:1.2}}>
            <i className="fas fa-bolt" style={{color:"#1ABB9C",marginRight:8}}></i>
        ...
        <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap"}}>
          {IS_ADMIN && (
            <span style={{padding:"4px 10px",borderRadius:4,background:"rgba(26,187,156,.1)",border:"1px solid rgba(26,187,156,.3)",color:"#1ABB9C",fontSize:11,fontWeight:700}}>
          ...
          <button className="mtz-abtn" onClick={()=>exportXLSX(filtered)} style={{...btnBase,background:"rgba(26,187,156,.08)",color:"#1ABB9C",border:"1px solid rgba(26,187,156,.3)"}}>
          ...
          {IS_ADMIN && <>
            ...
            <button className="mtz-abtn" onClick={()=>setEditRow("new")} style={{...btnBase,background:"#1ABB9C",color:"#fff"}}>
      ...
      <div style={{background:"#fff",border:"1px solid #E6E9ED",borderRadius:4,padding:"12px 14px",marginBottom:8,display:"flex",gap:12,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {["Todos",...TIPOS_ALL].map(t=>{ const c=TIPO_CFG[t]; const a=tipoF===t; return(
            <button key={t} className="mtz-fchip" onClick={()=>setTipoF(t)} style={{background:a?(c?.bg||"rgba(26,187,156,.1)"):"none",color:a?(c?.color||"#1ABB9C"):"#73879C",borderColor:a?(c?.color||"#1ABB9C"):"#CDD3D8"}}>
              {c?.icon&&c.icon+" "}{t}
            </button>);
          })}
        </div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {["Todas",...CRITS_ALL].map(c=>{ const cfg=CRIT_CFG[c]; const a=critF===c; return(
            <button key={c} className="mtz-fchip" onClick={()=>setCritF(c)} style={{background:a?(cfg?.bg||"rgba(26,187,156,.1)"):"none",color:a?(cfg?.color||"#1ABB9C"):"#73879C",borderColor:a?(cfg?.color||"#1ABB9C"):"#CDD3D8"}}>{c}</button>);
          })}
        </div>
      </div>
            ...
            <tr style={{background:"#F5F7FA"}}>
              {[
                ...
                <th key={col.l} className={col.k?"mtz-th-srt":""} onClick={col.k?()=>handleSort(col.k):undefined}
                    style={{padding:"10px 12px",textAlign:"left",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:".09em",color:"#98A6AD",borderBottom:"2px solid #1ABB9C",whiteSpace:"nowrap",width:col.w}}>
                ...
                <tr key={row.id} className="mtz-trow" style={{background:i%2===0?"#FFFFFF":"#F9FAFB",cursor:"pointer",borderBottom:"1px solid #E6E9ED"}} onClick={()=>setExp(isExp?null:row.id)}>
                  ...
                  <td style={{padding:"8px 12px"}}>
                    <div style={{color:"#555",lineHeight:1.4,fontSize:12}}>
                      ...
                      {!isExp && row.motivo.length>78 && <span style={{color:"#1ABB9C",fontSize:10,marginLeft:4}}>▾</span>}
                    </div>
                  </td>

...
async function verDetalle(id) {
  ...
  galeriaFotos=(d.evidencias||[]).map(ev=>`api/imagen.php?f=${encodeURIComponent(ev.ruta_imagen)}`);
  document.getElementById('modalDetalleBody').innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.08)">
      ...
      <div>
        ...
        ${d.evidencias.length?`<div style="display:grid;grid-template-columns:${d.evidencias.length===1?'1fr':'1fr 1fr'};gap:10px">${d.evidencias.map((ev,idx)=>`<div onclick="abrirGaleria(${idx})" style="border-radius:12px;overflow:hidden;cursor:pointer;position:relative;background:#111;border:2px solid rgba(255,255,255,0.08);transition:border-color 0.2s,transform 0.15s;aspect-ratio:${d.evidencias.length===1?'16/9':'4/3'}" onmouseover="this.style.borderColor='var(--amarillo)';this.style.transform='scale(1.02)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.08)';this.style.transform='scale(1)'"><img src="api/imagen.php?f=${encodeURIComponent(ev.ruta_imagen)}" style="width:100%;height:100%;object-fit:cover;display:block" onerror="onEvidenciaError(this)"><div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.8));padding:10px;display:flex;align-items:center;justify-content:space-between"><span style="font-size:11px;color:#fff;font-weight:600"><i class="fas fa-search-plus"></i> Foto ${idx+1}/${d.evidencias.length}</span><span style="font-size:10px;color:rgba(255,255,255,0.6)">Clic para ampliar</span></div></div>`).join('')}</div>`:`<div style="background:var(--gris-700);border-radius:10px;padding:40px;text-align:center;color:var(--gris-400)"><i class="fas fa-camera-slash" style="font-size:32px;margin-bottom:10px;display:block"></i>Sin evidencias fotográficas</div>`}
      </div>
    </div>

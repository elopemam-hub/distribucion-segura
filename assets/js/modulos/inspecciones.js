...
function renderTabla(rows) {
  ...
  tbody.innerHTML=rows.map(r => {
    ...
    return `<tr>
      ...
      <td><strong style="color:var(--amarillo);font-family:var(--font-display);font-size:15px">${escapeHtml(r.unidad)}</strong></td>
      <td class="muted">${escapeHtml(r.fecha)}<br><small>${escapeHtml(r.hora?.slice(0,5))}</small></td>
      <td>${escapeHtml(r.conductor)}</td>
      <td class="muted" style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(r.distrito)} · ${escapeHtml(r.provincia)}</td>
      <td><span class="badge ${bClass}">${pct}%</span></td>
      <td class="muted">${escapeHtml(r.inspector_nombre)||'—'}</td>
...
async function verDetalle(id) {
  ...
  document.getElementById('modalDetalleBody').innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:flex-start;margin-bottom:14px;padding-bottom:14px;border-bottom:2px solid rgba(21,101,192,0.2)">
      <div>
        <div style="font-family:var(--font-display);font-size:30px;font-weight:900;color:var(--amarillo);letter-spacing:2px;line-height:1">${escapeHtml(i.unidad)}</div>
        <div style="color:var(--gris-300);margin-top:6px;font-size:12px;display:flex;flex-wrap:wrap;gap:10px">
          <span><i class="fas fa-calendar-alt" style="color:var(--amarillo);margin-right:4px"></i>${escapeHtml(i.fecha)}</span>
          <span><i class="fas fa-clock" style="color:var(--amarillo);margin-right:4px"></i>${escapeHtml(i.hora?.slice(0,5))}</span>
          <span><i class="fas fa-map-marker-alt" style="color:var(--amarillo);margin-right:4px"></i>${escapeHtml(i.distrito)}, ${escapeHtml(i.provincia)}</span>
        </div>
        <div style="color:var(--gris-400);font-size:11px;margin-top:3px"><i class="fas fa-road" style="margin-right:4px;color:var(--amarillo)"></i>${escapeHtml(i.direccion)}</div>
      ...
      <div style="min-height:220px">${i.latitud&&i.longitud?`<div style="border-radius:12px;overflow:hidden;border:2px solid rgba(21,101,192,0.2);height:100%;min-height:220px;position:relative"><iframe src="https://maps.google.com/maps?q=${encodeURIComponent(i.latitud)},${encodeURIComponent(i.longitud)}&z=16&output=embed&hl=es" style="width:100%;height:100%;border:0;display:block;min-height:220px" loading="lazy"></iframe><a href="https://maps.google.com/?q=${encodeURIComponent(i.latitud)},${encodeURIComponent(i.longitud)}" target="_blank" style="position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.75);color:#fff;font-size:11px;padding:4px 10px;border-radius:20px;text-decoration:none;border:1px solid rgba(255,255,255,0.2)"><i class="fas fa-external-link-alt"></i> Ver en Google Maps</a></div>`:`<div style="border-radius:12px;background:var(--gris-700);height:100%;min-height:220px;display:flex;flex-direction:column;align-items:center;justify-content:center;border:1px dashed rgba(255,255,255,0.15);color:var(--gris-400);gap:10px"><i class="fas fa-map-marker-slash" style="font-size:32px"></i><span style="font-size:13px">Sin coordenadas GPS</span></div>`}</div>
    </div>
          ...
          return`<div style="background:var(--gris-700);border-radius:8px;padding:8px 10px;border-left:3px solid ${t.epp_completo?'var(--verde)':'var(--rojo)'}">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
              <div>
                ...
                <div style="font-weight:700;color:var(--gris-100);font-size:15px;margin-top:1px">${escapeHtml(t.nombre)}</div>
              </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.08)">
      ...
      <div>
        ...
        ${d.evidencias.length?`<div style="display:grid;grid-template-columns:${d.evidencias.length===1?'1fr':'1fr 1fr'};gap:10px">${d.evidencias.map((ev,idx)=>`<div onclick="abrirGaleria(${idx})" style="border-radius:12px;overflow:hidden;cursor:pointer;position:relative;background:#111;border:2px solid rgba(255,255,255,0.08);transition:border-color 0.2s,transform 0.15s;aspect-ratio:${d.evidencias.length===1?'16/9':'4/3'}" onmouseover="this.style.borderColor='var(--amarillo)';this.style.transform='scale(1.02)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.08)';this.style.transform='scale(1)'"><img src="${UPLOAD_URL}${ev.ruta_imagen}" data-fname="${escapeHtml(ev.ruta_imagen)}" style="width:100%;height:100%;object-fit:cover;display:block" onerror="onEvidenciaError(this)"><div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.8));padding:10px;display:flex;align-items:center;justify-content:space-between"><span style="font-size:11px;color:#fff;font-weight:600"><i class="fas fa-search-plus"></i> Foto ${idx+1}/${d.evidencias.length}</span><span style="font-size:10px;color:rgba(255,255,255,0.6)">Clic para ampliar</span></div></div>`).join('')}</div>`:`<div style="background:var(--gris-700);border-radius:10px;padding:40px;text-align:center;color:var(--gris-400)"><i class="fas fa-camera-slash" style="font-size:32px;margin-bottom:10px;display:block"></i>Sin evidencias fotográficas</div>`}
      </div>
    </div>

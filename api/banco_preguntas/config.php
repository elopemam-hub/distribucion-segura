...
$tiposValidos = ['manejo_practica', 'examen_defensiva', 'induccion_t2'];
if (!in_array($formulario, $tiposValidos, true)) {
    jsonResponse(false, 'Formulario inválido.', null, 400);
}

try {
    $secciones = db()->fetchAll(
        "SELECT id, seccion_id, titulo, descripcion, tipo, puntos
         FROM eval_secciones
    ...
    error_log('[banco_preguntas/config] ' . $e->getMessage());
